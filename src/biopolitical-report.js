...
(function attachBiopoliticalReport(root) {
  ...
  const COPY = {
    en: {
      ...
      analyticalReadiness: "Analytical readiness",
      readinessNote: "Coverage and evidence-integrity index; it is not permission to publish.",
    ...
    ar: {
      ...
      analyticalReadiness: "الجاهزية التحليلية",
      readinessNote: "مؤشر للتغطية ونزاهة الأدلة، وليس إذنًا بالنشر.",
    ...
    fr: {
      ...
      analyticalReadiness: "Préparation analytique",
      readinessNote: "Indice de couverture et d’intégrité probatoire ; ce n’est pas une autorisation de publication.",
  ...
  function build({ analysis, lang, version, bio, graphApi }) {
    ...
    const verified = evidence.filter((item) => item.verification_status === "verified").length;
    const verificationPct = evidence.length
    ...
    return `<!doctype html><html lang="${locale}" dir="${dir}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="app-version" content="${escapeHtml(version)}"><meta name="analysis-lens" content="biopolitical"><meta name="analysis-contract" content="${escapeHtml(bio.APP_CONTRACT)}"><meta name="schema-version" content="${escapeHtml(bio.SCHEMA_VERSION)}"><title>${escapeHtml(a.subject.title || c.title)}</title><style>${css}</style></head><body data-publication-gate="${gate}"><a class="skipLink" href="#report-content">${escapeHtml(c.skip)}</a><main class="shell" id="report-content" data-analysis-lens="biopolitical" data-export-contract-lens="biopolitical" data-analysis-contract="${escapeHtml(bio.APP_CONTRACT)}" data-schema-version="${escapeHtml(bio.SCHEMA_VERSION)}" data-app-version="${escapeHtml(version)}">
      <header class="hero"><div class="heroGrid"><div><p class="contractLine">${escapeHtml(c.officialContract)} · ${escapeHtml(bio.APP_CONTRACT)} · schema ${escapeHtml(bio.SCHEMA_VERSION)}</p><h1>${escapeHtml(a.subject.title || c.title)}</h1><p>${escapeHtml(c.title)}</p><p class="chain">${escapeHtml(c.chain)}</p></div><div class="statusPanel"><section class="metricCard"><strong>${score.overall}%</strong><span>${escapeHtml(c.analyticalReadiness)}</span><small>${escapeHtml(c.readinessNote)}</small></section><section class="metricCard"><strong>${verificationPct}%</strong><span>${escapeHtml(c.evidenceVerification)}</span><small>${verified}/${evidence.length} ${escapeHtml(c.verifiedEvidence)}</small></section><section class="metricCard"><span class="gateBadge">${escapeHtml(health.publishable ? c.passed : c.blocked)}</span><span>${escapeHtml(c.publication)}</span><small>${escapeHtml(health.publishable ? c.passedNote : c.blockedNote)}</small></section></div></div></header>
