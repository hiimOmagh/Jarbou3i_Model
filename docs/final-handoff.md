# Release Candidate Handoff — 2.0.0-bio-rc.15

This release candidate completes the automated intersection-audit layer around the Strategic and Biopolitical workbench and makes sample topics follow the selected interface language by default. RC 12 adds portable citation sanitation and a modular, truthful Biopolitical report; RC 13 preserves authored French punctuation; RC 14 completes 320px containment by constraining the explorer’s intrinsic grid sizing and measuring controls against clipping ancestors; RC 15 pins the Playwright runner containing the upstream worker-stop watchdog required for truthful Windows test completion. The Strategic v1.1 engine and canonical Biopolitical Training Map v2.1 schema remain unchanged. Canonical analyses with evidence warnings can be reviewed, but publication remains blocked until every warning is resolved. Lossless export and the strict verified-provenance gate remain intact.
...
The browser core contains 232 tests across Chromium, Firefox, WebKit, and mobile Chrome, including 48 reflow cases, eight sample-language linkage cases, import sanitation, modular-report export, and reference-resolution coverage. Hosted desktop/mobile evidence adds two execution cases and a separate evidence-review gate. Visual review adds 18 Chromium capture cases, 36 screenshots, 18 measurement records, and a separate 55-artifact evidence-review gate. The relationship explorer matrix retains spatial capability and mobile fallback, complete deterministic geometry, saved state, orbit and keyboard controls, reduced motion, runtime axe checks, temporal and comparative overlays, Focus, walkthroughs, persistent inspection, Story, Evidence Trail, Network, RTL, viewport-bound controls, and overflow protection.
