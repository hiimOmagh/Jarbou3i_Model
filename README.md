...
Version `2.1.0-alpha.39` stabilizes the operational review ledger transaction boundary and preserves focus across asynchronous workspace persistence. Review actions remain hash-chained local governance records rather than claims that an analysis is true. Canonical analysis export remains unchanged.
...
Alpha 4 expanded visual acceptance evidence to 96 deterministic screenshots, and Alpha 6 raised it to 120 screenshots and 139 artifacts. Alpha.38 adds the operational review ledger to every language/theme/viewport intersection, bringing the current contract to **138 screenshots and 157 artifacts** while retaining the five standalone-report surfaces. `npm run test:browser:hosted` remains a local public-surface gate when no URL is supplied; a release deployment must additionally pass `PLAYWRIGHT_BASE_URL=https://… npm run test:browser:deployed` so local evidence cannot be mistaken for deployed validation.
